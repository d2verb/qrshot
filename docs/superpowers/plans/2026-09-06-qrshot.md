...
各ターゲットのディレクトリが空なので、この時点で `swift build` は「ソースが無い」旨の警告に加えて `error: 'qrshot': target 'qrshot' referenced in product 'qrshot' is empty` というエラーも出す。それは正常で、Task 2 でソースが入れば解消する。ビルドの成否はまだ確認しない。

空ディレクトリは git が追跡しないため、このコミットはクリーンな clone からはビルドできない（＝ bisect 点にならない）。`.gitkeep` は追加しない。
...
**Files:**
- Create: `Sources/qrshot/main.swift`
