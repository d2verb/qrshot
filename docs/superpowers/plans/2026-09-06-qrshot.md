...
Expected: `Test run with 12 tests in 3 suites passed`
...
Expected: `Test run with 16 tests in 4 suites passed`
    ...
    case .copied(let decoded):
        if decoded.detectedCount > 1 {
            printToStandardError(
                "QR コードを \(decoded.detectedCount) 個検出したので、一番大きいものを使いました")
        }
        exit(0)
    case .notFound:
        exit(1)
...
5. **複数検出**: QR コードが 2 つ以上写る範囲を囲む。大きいほうの中身がコピーされ、stderr に「QR コードを N 個検出したので、一番大きいものを使いました」と出ること
````
...
Expected: 大きいほうの中身が `pbpaste` で取れる。stderr に「QR コードを 2 個検出したので、一番大きいものを使いました」が出る。
