...
> **注記（2026-09-06、実装完了後に追記）:** この計画はすでに実行済みで、qrshot は完成している。この文書は実行の記録として残しており、チェックボックスは意図的に未チェックのままにしてある（進行中という意味ではない）。実装の過程で計画時点では分からなかった事実が判明した箇所は、チェックを埋める代わりにタスク本文へ直接書き戻してある。
...
面積が最大の QR を採用する。`payloadString` が取れない QR（バイナリペイロード）は読み取り不能として次の候補へ送る。
...
実際に書かれた内容はリポジトリルートの `README.md` を参照。
